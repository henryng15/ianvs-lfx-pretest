...
**Terminal evidence video (1:10, no audio):** [watch or download the MP4](https://github.com/henryng15/ianvs-lfx-pretest/blob/main/evidence/videos/task2-reproduction.mp4). A continuous VS Code-style terminal walkthrough types each command at a readable pace and scrolls through its preserved observed output for the analysed SHA, `pr-558`, R4a, R4b, and the Ianvs runtime probe. The diagnostic errors shown are expected evidence, not a failed submission; the original full outputs remain in `evidence/*.txt`.
